    ...
    async getPendingTransactionCount(provider: Provider): Promise<number> {
        try {
            ...
            if (txpoolStatus !== null) {
                Logger.info('Used method: txpool_status');
            ...
            if (pendingCount !== null) {
                Logger.info('Used method: eth_getBlockTransactionCountByNumber("pending")');
                return pendingCount;
            }

            // Method 3: Fallback - estimate by checking latest block vs pending block
            Logger.info('Used method: eth_getTransactionCount("pending")');
